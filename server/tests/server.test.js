...
describe('PATCH /todos/:id', () => {
    ...
    it('should not update the todo of other user', (done) => {
        var updated = {text: "Updated second todo", completed: true}
        request(app)
            .patch(`/todos/${todos[0]._id.toHexString()}`)
            .send({text: updated.text, completed: updated.completed})
            .set('x-auth', users[1].tokens[0].token)
...
describe('POST /users', () => {
    ...
    it('should return validation errors if request invalid', (done) => {
        var emailOne = 'example';
        var passwordOne = '123456';
        var emailTwo = 'example@123.com';
        var passwordTwo = '123';
        request(app)
            .post('/users')
            .send({emailOne, passwordOne})
            .expect(400)
        request(app)
            .post('/users')
            .send({emailTwo, passwordTwo})
            .expect(400)
            .end(done);
    });
